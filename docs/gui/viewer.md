...
| 选中/编辑 | `selected_cell`、`selected_range`、`editing_cell`、`edit_value`、`just_entered_edit_mode`、`hovered_cell`、`drag_anchor`、`fill_drag_source`、`shift_click_anchor`、`copied_cells` | 单格选中、范围选中、正在编辑的格、框选拖拽锚点、填充柄拖拽源锚点、Shift+点击范围选择锚点、内部剪贴板缓冲（TSV） |
...
| 表格主体 | 单元格网格 | `selected_cell`/`selected_range`/`editing_cell`/`edit_value`/`hidden_*`/`shift_click_anchor`/`copied_cells` |
...
ui() CentralPanel
 │
 ▼ ScrollArea::both("table_scroll")
     ▼ draw_table_content(ui, excel_data, current_sheet, &mut 选中/编辑..., &hidden_*, &mut shift_click_anchor, &mut committed_paste, &mut copied_cells)
        ├─ 构建累积宽高数组（隐藏行列贡献 0）
