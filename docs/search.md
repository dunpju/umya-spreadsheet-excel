...
`Default` 初始化：`visible=false`、`collapsed=false`，并预置一条空的 `ColumnFilter{column_index:0, filter_value:"", logic:And}`。
...
3. **逐条计算匹配列集合**（调用 `compute_column_matches`，见 §5），同时累加 `max_total` 与 `any_binary`。
...
3. **解析所有筛选器关键字**为 owned 的 `ParsedFilter{ col, keywords, is_range }`（线程安全）。
...
6. **路径选择**：
   ...
   | 条件 | 路径 | `search_mode` |
   |------|------|---------------|
   | `!has_or && is_sorted` | **二分查找**（P0）：`find_rows_in_sorted` 在首列取候选行；多筛选器时对候选行验证其余列 | `"二分"` |
   | `row_count > PARALLEL_ROW_THRESHOLD (5000)` | **多线程并行线性扫描**（P2）：`std::thread::scope` 分块 | `"并行"` |
   | 否则 | **串行线性扫描** | `"串行"` |

   二分路径仅适用于**纯 AND 且首列已排序**——因为 OR 语义下首列未必匹配却仍可能命中。多筛选器时二分先缩小候选集，再对每个候选行用 `match_filter_value` 验证其余列（按需 `get_cell`，已被候选集大幅缩小）。
...
- **`logic_seq_ref`（=`&logic_seq`）**：仅在 `use_or` 分支被 `row_matches_expr` 消费，用于还原 AND/OR 的分组优先级（见 §5.2）。
...
```rust
fn match_filter_value(value, keywords, is_range) -> bool {
    if is_range && keywords.len() == 2 {
        v >= keywords[0] && v <= keywords[1]   // 闭区间范围匹配（字典序）
    } else {
        keywords.iter().any(|kw| row_fuzzy_match(value, kw)) // 任一关键字模糊命中
    }
}
```

- **范围模式**：闭区间 `>= lo && <= hi`，按字符串字典序比较（适合日期、编号等可排序值）。
- **模糊模式**：`row_fuzzy_match` = 大小写不敏感子串 `contains`。多值时 `any`（命中其一即可）。
- 所有列/行搜索路径（二分候选验证、串行、并行）都收敛到这一个函数，保证语义一致。

### 5.2 `row_matches_expr` —— AND/OR 表达式求值
...
draw_search_window  (UI：搜索按钮 / Enter 键 / 重置)
 ...
 │
 ├─【列筛选有输入】execute_multi_column_search ──► compute_column_matches
 │                                                  ├► find_sorted_column_matches（已排序）
 │                                                  └► 线性 contains（未排序）
 │                                                  写入 hidden_columns
 │
 ├─【行筛选有输入】execute_row_search
 │   ├─ parse_row_keywords → parsed (ParsedFilter)
 │   ├─ logic_seq / has_or
 │   ├─ collect_column_values (P1 预收集)
 │   ├─ 路径选择：
 │   │   ├─ find_rows_in_sorted（二分）
 │   │   ├─ std::thread::scope + local_hidden + row_matches_expr（并行）
 │   │   └─ 串行 + row_matches_expr
 │   ├─ expand_hidden_rows_for_merged_cells（合并对齐）
 │   └─ 写入 hidden_rows
 │
 └─ match_filter_value（所有路径的统一单值判定核心）
```

### 5.5 二分查找的细节（`find_sorted_column_matches` / `find_rows_in_sorted` / `search_sorted`）
...
### 6.6 重置按钮
统一清空：`hidden_columns`/`hidden_rows`、列搜索状态（`is_searching/matched_count/total_searched/search_keyword`）、行筛选状态及所有 `row_filters[i].keyword`、`row_debug_info`，并把 `column_filters` 复位为一条空条件（保留交互骨架）。
...
`draw_search_window` 绘制一个 **固定宽度 440px、自定义标题栏、非模态** 的 egui 浮窗。
...
│                                                                          │
│  日期 (A14): [ xxxx 或 'xx1','xx2' 或 'xx3'-'xx4'         ] [AND ▾]      │ ← 行筛选项①
│  入库 (D14): [                                              ] [OR ▾]      │   输入框250
│                                                                          │
...
| 标题栏 | `🔄 重置` | 清空 `hidden_*`、搜索状态、关键字，`column_filters` 复位为一条 | — |
...
| 行筛选项 | `标题 (cell_ref):` + TextEdit | `row_filters[idx].keyword`（清空自动还原；Enter 触发搜索） | 250 |
...
| P2 并行扫描 | `std::thread::scope` + `local_hidden` | >5000 行时分块并行，无锁合并 |
| 自适应路径选择 | `execute_row_search` | 按排序性 / 行数自动选最优路径 |
| 短路求值 | `row_matches_expr` 的 OR 短路 / `all(...)` | 命中即停，减少无效比较 |
| 阈值常量 | `PARALLEL_ROW_THRESHOLD=5000`、`MIN_ROWS_PER_THREAD=2000` | 控制并行粒度，避免线程过细 |
