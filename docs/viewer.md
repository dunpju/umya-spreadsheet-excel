...
- **快捷键与撤销**：`Ctrl+S/Z`、`Delete`、`Escape` 等全局键绑定，以及多粒度撤销栈。
...
| 选中/编辑 | `selected_cell`、`selected_range`、`editing_cell`、`edit_value`、`just_entered_edit_mode`、`hovered_cell`、`drag_anchor` | 单格选中、范围选中、正在编辑的格、拖拽锚点 |
...
| `start_async_load(path, ctx)` | 开线程 `ExcelData::load_from_file`，`tx`/`rx` 通道，置 `LoadState::Loading`，完成后 `ctx.request_repaint()` |
...
| `check_save_result()` | 每帧 `save_rx.try_recv()`：成功置 `save_path` + `dirty=false`，失败置 `error_message` |
...
ui(&mut self, ui, frame)                                          // 每帧入口
 ...
 ├─ 每帧 reapply_conditional_formatting（文件自带 + 用户规则）
 ├─ draw_convert_popup
 ...
 │
 ├─ check_load_result() / check_save_result()                     // 回收异步结果
 ...
 │   ├─ pending_undo = Ctrl+Z ? take_undo() : None                 // 借用 excel_data 前取出
 │   ├─【有数据】
 ...
 │   └─【无数据】LoadState ► Loading(spinner+request_repaint) / Failed/Idle(draw_empty_state)
 │
...
| 表格主体 | 单元格网格 | `selected_cell`/`selected_range`/`editing_cell`/`edit_value`/`hidden_*` |
...
| 事件 | 检测 | 行为 |
|------|------|------|
| 左键单击 | `response.clicked()` | 更新 `selected_cell`（`col,row>0` 才有效）；`request_focus` |
...
| 键 | 行为 |
|----|------|
| `←/→/↑/↓` | 单格移动 `selected_cell`，越界自动 `scroll_to_rect`（冻结区偏移补偿） |
| `Tab` / `Shift+Tab` | 编辑态下提交并右/左移；非编辑态下右/左移选中 |
...
| `Ctrl+Z`（无 Shift） | CentralPanel | `take_undo()` 回放（借出在 `excel_data` 之前） |
...
菜单"导入" ──► draw_import_dialog(返回 path)
   ...
   └─ spawn thread: ExcelData::load_from_file(path) ──► tx.send(Ok/Err) ──► ctx.request_repaint()
        │
...
ui() CentralPanel
 │
 ▼ ScrollArea::both("table_scroll")
     ▼ draw_table_content(ui, excel_data, current_sheet, &mut 选中/编辑..., &hidden_*)
        ├─ 构建累积宽高数组（隐藏行列贡献 0）
...
Ctrl+S（dirty&&!saving&&有数据）/ 名称框"💾 保存" ──► save_requested=true
   ...
   └─ spawn thread: writer::save_to_file(原路径, 数据, 输出路径) ──► tx.send ──► request_repaint
        │
        ▼ 每帧 check_save_result() ── save_rx.try_recv()
        ├─ Ok(path): save_path=Some(path); dirty=false ──► 状态栏绿色新路径
        └─ Err(e):   error_message=Some(e)
```

> 关键点：① **保存校验点前置**（拦截态禁保存）；② **副本落盘**（克隆 `ExcelData` 给后台线程，
> 主线程继续编辑不阻塞）；③ 输出**带日期后缀的新文件**，不动原模板。
