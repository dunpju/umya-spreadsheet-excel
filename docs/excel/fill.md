...
- **向下**：在相邻列（**先左** `start_col-1`，**后右** `end_col+1`）中，取「源末行下一行 `end_row+1`」处有数据的那列作锚点；从该处起 [`scan_down`] 向下扫**连续非空格**，末行即目标行。紧邻位（`end_row+1`）即空 → 该方向无边界。
- **向右**：在锚点行（**源行优先** `start_row`，再相邻行**先上** `start_row-1`、**后下** `end_row+1`）中，取「源末列右一列 `end_col+1`处有数据」的那行作锚点；从该处起 [`scan_right`] 向右扫连续非空格，末列即目标列。源行优先确保单行横向选区（数据在同行延伸）正确识别边界。

> 用例对齐：横向 `AH1:AI1=17`/`AJ1:AK1=18`，相邻第 2 列数据延伸到 `AN` 列 → 向右填到 `AN1`（19,20,21）；纵向 `A38="08月17号"`/`A39="08月18号"`，相邻 B 列数据延伸到第 44 行 → 向下填到 `A44`（`08月19号`…`08月23号`）。
...
`#[cfg(test)] mod tests` 覆盖：单数字递增、等差序列（`1,2,3→4,5,6`）、等差步长 2（`2,4→6,8`）、等比（`2,4,8→16,32`）、水平填充、反向（向上）、文本复制、公式相对平移、`$` 绝对不变、返回 `old_cells` 用于撤销、中文日期文本递增（`08月24号→08月25号`、月末跨月、含/不含年份）、纯文本不被误判为日期、**合并单元格水平填充不被空合并体污染（报修用例：合并值 18 → 19 而非 -18）**、**目标为合并格时只写左上角**；以及双击自动填充目标推断：**横向合并源填到相邻边界**（`A1:D1` 合并 17/18 → `E1:G1`=19/20/21）、**纵向中文日期文本填到边界**（`A1:A2` → `A6`）、**相邻数据含合并**（横向/纵向）、**无相邻数据→None**、**上限夹紧**（6 万行连续数据夹到 5 万）、**隐藏行透明**、**横向线不回退纵向**（右侧无数据→None）、**纵向线不回退横向**（下方无数据→None）；以及 `compute_fill_values`：**数字垂直预计算**、**预计算结果与 apply_fill 一致**、**水平预计算**、**日期文本预计算**、**合并场景预计算不被污染**、**公式预计算**、**target 在源内返回 None**。
