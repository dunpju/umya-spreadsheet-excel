...
- **序列推断与填充**：公式（相对引用平移）、日期（按天递增）、数字（算术/等比）、中文日期文本（`08月24号` 等按天递增）、数字型日期文本（`2026/2/7`、`2026-01-26` 等按天递增）、纯文本（复制）。
...
| **数字型日期文本** | `value` 匹配 `YYYY<M><D>`，分隔符为 `/` 或 `-`（如 `2026/2/7`、`2026-01-26`） | 按天递增 `base ± (k+1)·d`（`d` 由源序列推断，默认 1 天）；输出经 `format_date(serial, fmt)` 按**源格原格式**（分隔符 + 月/日前导零）回填、`raw_number = Some(serial)` |
...
>
> **数字型日期文本识别（`parse_numeric_date`）**：无 `number_format`、非中文的斜杠/连字符日期（如用户在空格里键入的 `2026/2/7`、`2026-01-26`，编辑器以纯文本保存）也不被 `is_date_format` 识别，故独立识别：**仅年开头、年份 4 位（≥1000）**（避免与 `m/d/yyyy`、两位年份歧义），分隔符 `/` 或 `-`（不可混用），无效日期（如 `2026/2/30`）经序列号往返校验后拒绝。`fmt` 记录分隔符与前导零（`yyyy/m/d`、`yyyy-mm-dd` 等），递增后由 `format_date` 按原格式回填。
...
- **向下**：从源末行下一行（`end_row+1`）起，**仅沿源列**（`start_col..=end_col`）查找第一个有数据的行，从该行起向下扫描连续非空格，末行即目标行。可跳过起始空行。**若源列全空**（如源位于表格最底部数据），则兜底填充到 `max_row`。
- **向右**：从源末列右一列（`end_col+1`）起，在候选行（源行优先 → 向上 ≤10 行 → 向下 ≤10 行）中取首个有数据的行作锚点，向右扫描连续非空格，末列即目标列。数据延伸行常在源行之外（如横向选区同行无数据但第 2 行有数据）。
- **单格/方块**：方向不明确，允许搜索相邻列（左右各 ≤10 列）/相邻行（上下各 ≤10 行）以推断边界。

> 用例对齐：横向 `AH1:AI1=17`/`AJ1:AK1=18` + 第 2 行数据延伸到 `AN` → 向右填到 `AN1`（19,20,21）；纵向 `A38="08月17号"`/`A39="08月18号"` + 源列 A 数据延伸到第 44 行 → 向下填到 `A44`（`08月19号`…`08月23号`）；纵向 `A44="08月23号"`/`A45="08月24号"` + 表格末行 47、源列下方全空 → 兜底填到 `A47`（`08月25号`…`08月26号`）。
...
`#[cfg(test)] mod tests` 覆盖：单数字递增、等差序列（`1,2,3→4,5,6`）、等差步长 2（`2,4→6,8`）、等比（`2,4,8→16,32`）、水平填充、反向（向上）、文本复制、公式相对平移、`$` 绝对不变、返回 `old_cells` 用于撤销、中文日期文本递增（`08月24号→08月25号`、月末跨月、含/不含年份）、**数字型日期文本递增（`2026/2/7→2026/2/8`、连字符+前导零 `2026-01-26→2026-01-27`、月末跨月、无效日期 `2026/2/30` 不被误判、写入 `raw_number`）**、纯文本不被误判为日期、**合并单元格水平填充不被空合并体污染（报修用例：合并值 18 → 19 而非 -18）**、**目标为合并格时只写左上角**；以及双击自动填充目标推断：**横向合并源填到相邻边界**（`A1:D1` 合并 17/18 → `E1:G1`=19/20/21）、**纵向中文日期文本填到边界**（`A1:A2` → `A6`）、**纵向日期文本精确行号**（`A38:A39` → `A44`=08月19号..08月23号）、**纵向跳过起始空行**（A40 空、A41:A44 有数据→正确填充）、**纵向兜底填充到 max_row**（A44:A45 源、A46/A47 空、max_row=47→填到 A47）、**相邻数据含合并**（横向/纵向）、**无相邻数据→None**、**上限夹紧**（6 万行连续数据夹到 5 万）、**隐藏行透明**、**横向线不回退纵向**（右侧无数据→None）、**纵向线不回退横向**（下方无数据→None）、**横向源行限定锚点**（仅扫描源行、忽略非源行数据）、**纵向源列限定锚点**（仅扫描源列、忽略非源列数据）；以及 `compute_fill_values`：**数字垂直预计算**、**预计算结果与 apply_fill 一致**、**水平预计算**、**日期文本预计算**、**数字型日期文本预计算（斜杠/连字符）**、**合并场景预计算不被污染**、**公式预计算**、**target 在源内返回 None**；以及**数字型日期双击自动填充端到端**（`A1=2026/2/7`/`A2=2026/2/8` + 相邻数据定界 → 填到边界、按天递增）。
